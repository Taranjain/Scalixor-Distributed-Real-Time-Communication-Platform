...
export async function subscribeToRoom(roomId: string): Promise<void> {
    try {
        await subscriber.subscribe(roomChannel(roomId));
        log(PORT, `✅ Subscribed to room channel: ${roomChannel(roomId)}`);
    } catch (err) {
        log(PORT, `❌ Failed to subscribe to room ${roomId}: ${err}`);
    }
}
...
export async function unsubscribeFromRoom(roomId: string): Promise<void> {
    try {
        ...
        log(PORT, `✅ Unsubscribed from room channel: ${roomChannel(roomId)}`);
    } catch (err) {
        log(PORT, `❌ Failed to unsubscribe from room ${roomId}: ${err}`);
    }
}
...
export async function publishToRoom(roomId: string, payload: object): Promise<void> {
    try {
        ...
        log(PORT, `❌ Redis room publish failed: ${err}`);
    }
}
