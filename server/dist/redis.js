...
async function subscribeToRoom(roomId) {
    try {
        await exports.subscriber.subscribe(roomChannel(roomId));
        (0, utils_1.log)(PORT, `✅ Subscribed to room channel: ${roomChannel(roomId)}`);
    }
    catch (err) {
        (0, utils_1.log)(PORT, `❌ Failed to subscribe to room ${roomId}: ${err}`);
    }
}
async function unsubscribeFromRoom(roomId) {
    try {
        await exports.subscriber.unsubscribe(roomChannel(roomId));
        (0, utils_1.log)(PORT, `✅ Unsubscribed from room channel: ${roomChannel(roomId)}`);
    }
    catch (err) {
        (0, utils_1.log)(PORT, `❌ Failed to unsubscribe from room ${roomId}: ${err}`);
    }
}
/* ================================
   PUBLISH HELPERS
...
async function publishToRoom(roomId, payload) {
    try {
        await exports.publisher.publish(roomChannel(roomId), JSON.stringify(payload));
    }
    catch (err) {
        (0, utils_1.log)(PORT, `❌ Redis room publish failed: ${err}`);
    }
}
